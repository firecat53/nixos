...
}:
{
  ...
  services.prometheus = {
    ...
    # Written as a structured attrset and serialized with builtins.toJSON.
    # Prometheus reads rule files as YAML, and JSON is valid YAML, so this is
    # equivalent to a .yml rules file but with no hand-managed indentation or
    # string escaping. promtool still validates it at build time.
    rules = [
      (builtins.toJSON {
        groups = [
          {
          ...
          }
          {
            ...
            rules = [
              ...
              }
              {
                ...
                # Exclude dm- (encrypted volumes) and sdf (external drive) from
                # this tolerance as they are frequently slow.
              ...
              }
              {
                ...
                # Include only dm- (encrypted volumes) and sdf (external drive)
                # for this tolerance as they are frequently slow.
          ...
          # Per-vdev error counts come from the zfs-error-exporter textfile
          # metrics (zfs-error-exporter.nix) since the zfs_exporter does not
          # expose them.
          {
            name = "zfs";
            rules = [
